...
class AdapterPattern
{
    ...
    //Adapter 
    //Also, in the proxy pattern, it is typical for the Proxy to extend the subject class, so MyOpenBook is acting as the subject here
    public class MyCoolBook : MyOpenBook
    {
        //By definition in the adatper pattern, the adapter adapts the interface adaptee
        //to the target interface.  In this case we reference visuals, an Interact object.
        //Interact extends forms which needs to be adapted to work as a CoolBook user's wall
        ...
        //This send gift method goes here because each adaptee can use it, the adaptee sets up
        //details for the button that triggers this method so it
        //is set up here in the adapter like the version of the poke method
        //that is associated with clicking the Poke button.  
    ...
    //By definition in the adapter class, the Adaptee defines an
    //existing interface that needs adapting.  In this case, Interact
    //extends Form which needs to be adapted to serve as a CoolBook
    //"wall" for a user.
    ...
    // The real subject is what the proxy represents.  Although in this case
    // SpaceBook does not extend the subject class directly as is typical of
    //the proxy pattern.  
    // CANNOT CHANGE
    ...
    //CANNOT CHANGE
    //This class is the one that is used to instantiate objects in the Main() method via
    //the fact that MyCoolBook extends MyOpenBook.  By definition in the adapter pattern,
    ...
    // The Client
    //Can create different objects that conform to MyCoolBook, taking a name
    //as input.  That is common, as in the adapter class you want to be
    //able to instantiate and call methods of objects in a common way.
    //SpaceBook is the real subject, MyCoolBook can be used as a proxy
